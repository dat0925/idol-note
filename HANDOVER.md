...
### まだやっていないこと（次にやるべきこと）
1. **Supabase の Redirect URLs に2件を追加する**（上記「Supabase 側に必要な設定」）
   → これが済むまで Google ログインは taskra に着地して戻ってこない
2. 親・娘の2アカウントで実際にログインし、アプリ経由で
   `idol_create_family` / `idol_join_family` を通す
   - 親: mstd0520@gmail.com（Google 認証の成功までは確認済み）
   - 娘: kyoka.endo1006@gmail.com（この端末の Chrome には未ログイン）
   ※上の突破テストは **DB層（Postgres の RLS）** の検証。
     PostgREST 経由・PIN・画面遷移まで含めた通しの確認はまだ
3. PIN の設定と、忘れた場合の Google 再ログイン導線の実地確認

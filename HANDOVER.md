...
- 本文 `#3a2b36`: **7.6〜9.05** → AAA を満たす
- 補助文字 `#8a7684`（旧）: **2.4〜2.86** → 大きく不足
  - ※これは元からの問題で、**真っ白なカードの上ですら 4.2** と AA(4.5) 未満だった
  - → こどもモードの `--text-sub` を `#5b4a54` に変更（背景の上 4.71 / 白の上 8.23）

★おとなモードの `--text-sub` は `#8a7684` のままで **4.2**。まだ基準未満なので、
気づいたら直すこと（今回はこどもモードの透過が主題だったため触っていない）。
...
### おとなモードのスマホ表示を修正（2026-08-16）

390px 幅で崩れていた2画面を直した。DB・RLS には触れていない（見た目のみ）。

| 画面 | 症状 | 対処 |
|---|---|---|
| 目標とロードマップ | タイトルが1文字幅に潰れて縦書き状態 | `js/views/goals.js` の1行を `tree__meta` / `tree__actions` の入れ物に分け、700px以下で grid の2段組みに |
| カレンダー | 予定名の長い日があると土曜の列が画面外へ消える | `.cal` を `minmax(0, 1fr)` に。スマホでは予定を色帯だけにして、名前は日付タップの日別パネルで読ませる |

検証は「実データが要らない静的ハーネス」を作り、実CSSを読み込んで
360 / 430 / 701 / 768 / 1200px でヘッドレスChromeから確認した
（`document.scrollWidth > innerWidth` と、幅40px未満かつ高さ60px超のテキスト要素の数で判定）。
おとな画面はログイン＋PIN が要るので、この方式なら崩れの検証だけを切り出せる。
ホーム / 練習記録 / オーディションは同じ検査で問題なし
（表は `.table-wrap` の横スクロールに載っていて意図どおり）。
...
- **flex / grid の子は「min-content 幅」より縮まない（2026-08-16 に2件踏んだ）**
  日本語は1文字ずつ折り返せるので min-content = 1文字幅。つまり
  **潰れても壊れずに縦書きのようになってしまい、はみ出し検査に引っかからない**。
  - おとなの目標ツリー: `.tree__title { flex: 1 }` だけでは、幅の決まった兄弟
    （バー96px・％34px・ボタン4個）に押されてタイトルが14px幅まで潰れた。
    → `min-width: 0` を足し、さらに700px以下では grid で2段に組み替えた
  - おとなのカレンダー: `repeat(7, 1fr)` は `minmax(auto, 1fr)` と同義で、
    予定名（`white-space: nowrap`）の長い日があるとその曜日の列だけ膨らみ、
    **7列目（土）が画面外に押し出されて消えた**。→ `minmax(0, 1fr)` に変更
  検証は「横スクロールが出るか」だけでは不十分。**幅が極端に狭く縦に長い
  テキスト要素**も併せて洗い出すこと（`width < 40 && height > 60` で拾える）。
- **`.tree--big > .tree__row` のような子孫セレクタは、同じ要素に両方のクラスが
  付いている場合に一致しない**。`class="tree__row tree--big"` なので
  正しくは `.tree__row.tree--big`。大目標だけ大きくする指定が丸ごと死んでいた。

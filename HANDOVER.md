...
★ブラウザ実機（Supabase につないだ状態）での画面確認はまだ。
　この環境からは Supabase / CDN に出られないため、**目視の最終確認が残っている**。

#### ピアノのロードマップは投入済み（2026-08-16）
本番日 `2026-12-26` で **34件（最終1 / 中間4 / 月5 / 行動24）を DB に投入済み**。
アプリの「雛形から作る」と同じ内容を `goal-templates.js` から SQL を起こして入れた。
**もう一度アプリ側で雛形を作ると二重になる**ので押さないこと。

投入後に実測で確認したこと:
- 階層（big → milestone → month → task）が親子で正しくつながっていること
- **ロールアップが4段とも効くこと**（トランザクション内で確認し rollback 済み）。
  8月の行動目標 5件中2件を完了 → 月 40% → 中間 20% → 最終 5%。
  新しく足した `milestone` の段でも平均が正しく伝わる
- **娘のセッション（RLS）を再現した突破テスト**（CLAUDE.md の手順）
  → 目標 34件は見える（家族共有なので正しい）／
    `idol_body_records` `idol_parent_pins` `idol_audition_results` はすべて 0件

#### 生年月日も投入済み（2026-08-16）
本人（きょうか）の `birthday = 2017-10-06`。
アプリの `ageAt()` と Postgres の `age()` が一致することを確認した
（本番 2026-12-26 の時点で **9歳2か月**、投入時点で 8歳10か月。10/6 に9歳になる）。
親側の生年月日は入れていない（使い道がないので不要）。
